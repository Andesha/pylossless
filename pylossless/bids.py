...
def convert_recording_to_bids(
    ...
    **write_kwargs
    ...
    import_func : function
       ...
       dictionary (specified as usual in MNE-Python). If `import_events` is
       set to False, this function must return an object of the `mne.io.Raw`
       class.
    import_kwargs : dict
    ...
    bids_path_kwargs : dict
       Dictionary of the keyword arguments necessary to be passed to the
...
def convert_dataset_to_bids(
    ...
    **write_kwargs
    ...
    import_fct : function or list of functions
       ...
       dictionaries in the `import_args` list. If `import_events` set to True,
       this function must return 1) an object of type `mne.io.Raw`, 2) a
       structure of events (specified as usual in MNE-Python), and 3) a
       event_id dictionary (specified as usual in MNE-Python). if
       `import_events` set to False, this function must return an object of the
       `mne.io.Raw` class.  This argument can also be a list of functions, in
       case where, for example, different recordings have different formats and
       require different import functions. In this case, the length of this
       attribute must be the same as `import_args`.
