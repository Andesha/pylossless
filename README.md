...
2. Rejection of bad channels followed by rejection of bad time segments
3. Configurable high and low pass filtering
...
Below is a minimal example that runs the pipeline with one of MNE's sample files.

Be aware that this example will return a warning that the sampling rate of the data is too low for ICLabel. This is an expected property of the sample data and will not occur in real world data.
...
Once the whole pipeline is completed, You can see what channels and times were flagged based on the below code block.
```python
...
0. Follow the above section on creating the HPC environment
...
# Automatically select 10 second windows of time with no events and mark them as "bad"
# This simulates marking portions of the recording as "out of task"
...
# Save pipeline output as a BIDS derivative in the current directory
...
# The $@ convention is what allows us to pass arguments into this job script from the subsequent step
python main.py $@
```

5. Create the final submission bash script, typically called `run_all.sh`. This script can be generated by spreadsheet, or other tool and calls `job.sh` on each subject file. As seen below, information after the `job.sh` argument is passed into the `main.py` call. The intention being that changing all job parameters is only done once. Further, this names all jobs consistently to better monitor potential failures. Other arguments in each line name the jobs, and redirect their output and errors to one helpful location. Make sure this is created at the root level with `mkdir logs`. Note that the subject ids, file names, etc are intended to be changed to match a given study.
